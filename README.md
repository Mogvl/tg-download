...
下载进度按频道持久化在 `config.yaml` 的 `chat[].last_read_message_id`。

- **整频道下载完成后**：游标推进到该频道最新消息，下次启动不再重扫。
- **下载到一半重启容器**：已成功下载的消息 id 也会实时回写游标，重启后从**断点继续**，不会从频道最旧消息重头扫（已存盘的文件会被自动识别并跳过，不重复下载、不损坏数据）。

> 如需让某个频道重新从头下载，可手动把该频道 `last_read_message_id` 改为 `0` 并重启容器。
